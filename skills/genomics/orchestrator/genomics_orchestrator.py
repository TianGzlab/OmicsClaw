...
def _get_keyword_map() -> dict[str, str]:
    ...
    return registry.build_keyword_map(domain="genomics", fallback_map=_FALLBACK_KEYWORD_MAP)
...
def route_query(query: str) -> dict:
    ...
    return {"matched": False, "skill": "genomics-qc", "confidence": 0.0, "matched_keywords": [],
...
def route_query_with_mode(query: str, routing_mode: str = "keyword") -> dict:
    ...
    if routing_mode in ["llm", "hybrid"]:
        effective_map = _get_keyword_map()
        skill, conf = route_query_unified(query, effective_map, SKILL_DESCRIPTIONS, "genomics", routing_mode)
        if skill:
