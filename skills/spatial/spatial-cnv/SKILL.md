...
metadata:
  omicsclaw:
    ...
    param_hints:
      infercnvpy:
        priority: "reference_key → window_size → step"
        params: ["reference_key", "reference_cat", "window_size", "step"]
        defaults: {window_size: 100, step: 10}
        requires: ["X_log_normalized"]
      numbat:
        priority: "reference_key/reference_cat"
        params: ["reference_key", "reference_cat"]
        requires: ["counts_or_raw", "allele_counts_if_available"]
    legacy_aliases: [cnv]
    saves_h5ad: true
...
- **Why OmicsClaw**: Combines CNV inference, spatial mapping, and wrapper-generated output guidance so results are easier to interpret and reproduce

## Workflow

1. **Load**: Read the preprocessed h5ad and confirm spatial coordinates are available.
2. **Validate**: Check method-specific inputs such as raw counts, reference categories, and optional allele counts.
3. **Infer CNV**: Run expression-based `infercnvpy` or haplotype-aware `numbat`.
4. **Summarize**: Aggregate chromosome-arm or segment-level CNV scores and generate spatial figures.
