...
- **Why OmicsClaw**: Handles the full integration pipeline from multi-sample h5ad to corrected embeddings with consistent reports and wrapper-generated output guidance

## Workflow

1. **Load**: Read the preprocessed multi-sample AnnData and verify the batch column exists.
2. **Validate**: Check that PCA embeddings and upstream preprocessing outputs are available.
3. **Integrate**: Run Harmony, BBKNN, or Scanorama on the shared latent space.
