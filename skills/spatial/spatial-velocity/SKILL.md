...
- **Why OmicsClaw**: Integrates velocity vectors with spatial coordinates while preserving structured outputs and wrapper-generated guides for inspection and reruns

## Workflow

1. **Load**: Read the AnnData object and confirm spliced/unspliced layers are available.
2. **Preprocess**: Run the shared scVelo preprocessing path to compute normalized layers, moments, and the neighbor graph.
3. **Infer velocity**: Run the selected stochastic, deterministic, dynamical, or VELOVI model.
4. **Project and summarize**: Build velocity graph outputs and generate spatial / UMAP visualizations when possible.
