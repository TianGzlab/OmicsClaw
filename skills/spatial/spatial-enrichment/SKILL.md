...
metadata:
  omicsclaw:
    ...
    param_hints:
      enrichr:
        priority: "groupby → source"
        params: ["groupby", "source", "species", "gene_set"]
        defaults: {groupby: "leiden", source: "GO_Biological_Process", species: "human"}
        requires: ["ranked_or_filtered_gene_lists"]
      gsea:
        priority: "groupby → source"
        params: ["groupby", "source", "species", "gene_set"]
        defaults: {groupby: "leiden", source: "GO_Biological_Process", species: "human"}
        requires: ["ranked_gene_list"]
      ssgsea:
        priority: "groupby → source"
        params: ["groupby", "source", "species", "gene_set"]
        defaults: {groupby: "leiden", source: "GO_Biological_Process", species: "human"}
...
- **Why OmicsClaw**: Integrates directly with spatial DE results while keeping outputs consistent and easier to inspect through wrapper-generated guides and notebooks

## Workflow

1. **Load**: Read the clustered or annotated AnnData and identify the requested grouping column.
2. **Rank genes**: Build per-group marker lists or ranked gene lists as required by the chosen method.
3. **Run enrichment**: Execute built-in ORA, GSEA, or ssGSEA against the selected gene-set source.
4. **Summarize**: Collect significant pathways and generate enrichment plots and tables.
