...
author: OmicsClaw
license: MIT
tags: [singlecell, doublet, scrublet, doubletdetection, doubletfinder, scdblfinder,
  scds, qc]
metadata:
  omicsclaw:
    domain: singlecell
    ...
    param_hints:
      scrublet:
        ...
        - "--batch-key: useful when captures/samples are mixed and Scrublet should
          run per batch."
      ...
      doubletdetection:
        ...
        - "--method doubletdetection: consensus Python path borrowed from SCOP's method
          surface."
        - "The current wrapper records expected_doublet_rate for context, but the
          native DoubletDetection classifier does not use it directly."
      doubletfinder:
        ...
        - "If the R runtime fails, the wrapper falls back to scDblFinder and reports
          both methods."
      scdblfinder:
        priority: "expected_doublet_rate"
        params: ["expected_doublet_rate"]
      ...
      scds:
        ...
        - "--scds-mode chooses which score (`hybrid`, `cxds`, or `bcds`) becomes the
          public call surface."
