...
│  │  detect_domain() → route_query() → dispatch directly to skill    │     │
│  └───────────────────────────────────────────────────────────────────┘     │
└─────────────────────────────────────────────────────────────────────────────┘
                               │
                               ▼
...
│                                                                             │
│  skills/                                                                    │
│  ├── spatial/ (15 skills)           ├── singlecell/scrna/ (13 skills)      │
...
| 7 | 如需增强显式关键词路由，补充顶层 orchestrator fallback | `omics_orchestrator.py` | 手动 |
| 8 | 更新 CLAUDE.md 路由表 | `CLAUDE.md` | 手动 |
| 9 | 编写测试 | `tests/` | 手动 |

**核心结论：步骤 4 是自动的（Registry FS 扫描），但步骤 5-8 仍需要手动补充别名、安全映射、路由提示和测试。**
