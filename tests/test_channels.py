...
class TestMessageTypes:
    """Test InboundMessage and OutboundMessage data classes."""

    def test_inbound_message_creation(self):
        from bot.channels.bus import InboundMessage
        msg = InboundMessage(
            channel="telegram",
            sender_id="user123",
            chat_id="chat456",
    ...
    def test_inbound_message_defaults(self):
        from bot.channels.bus import InboundMessage
        msg = InboundMessage(channel="test", sender_id="u", chat_id="c", content="x")
    ...
    def test_outbound_message_creation(self):
        from bot.channels.bus import OutboundMessage
        msg = OutboundMessage(
            channel="feishu",
            chat_id="chat789",
            content="Response",
...
class TestMessageBus:
    """Test the async MessageBus."""

    def test_inbound_publish_consume(self):
        from bot.channels.bus import InboundMessage, MessageBus
    ...
    def test_outbound_publish_consume(self):
        from bot.channels.bus import MessageBus, OutboundMessage
    ...
    def test_subscriber_routing(self):
        from bot.channels.bus import MessageBus, OutboundMessage
        received = []
        ...
        async def _test():
            bus = MessageBus()
            bus.subscribe_outbound("telegram", _handler)
            msg = OutboundMessage(channel="telegram", chat_id="c", content="hi")
            await bus.publish_outbound(msg)

            # Manually dispatch one round
            out = await bus.consume_outbound()
            subscribers = bus._outbound_subscribers.get(out.channel, [])
    ...
    def test_uptime(self):
        from bot.channels.bus import MessageBus
        bus = MessageBus()
        assert bus.uptime_seconds >= 0
...
class TestDedupMiddleware:
    """Test DedupMiddleware."""

    def test_first_message_passes(self):
        from bot.channels.bus import InboundMessage
        from bot.channels.middleware import DedupMiddleware

        async def _test():
            mw = DedupMiddleware()
            msg = InboundMessage(
                channel="test", sender_id="u", chat_id="c",
                content="hello", message_id="msg1",
            )
            result = await mw.process(msg)
    ...
    def test_no_id_always_passes(self):
        from bot.channels.bus import InboundMessage
        from bot.channels.middleware import DedupMiddleware
        ...
        async def _test():
            ...
            msg = InboundMessage(
                channel="test", sender_id="u", chat_id="c",
                content="hello", message_id="",
            )
            r1 = await mw.process(msg)
            r2 = await mw.process(msg)
...
class TestRateLimitMiddleware:
    ...
    def test_under_limit_passes(self):
        from bot.channels.bus import InboundMessage
        from bot.channels.middleware import RateLimitMiddleware

        async def _test():
            mw = RateLimitMiddleware(max_per_hour=5)
            for i in range(5):
                msg = InboundMessage(
                    channel="test", sender_id="user1", chat_id="c",
                    content=f"msg{i}",
                )
                result = await mw.process(msg)
                assert result is not None
    ...
    def test_over_limit_dropped(self):
        from bot.channels.bus import InboundMessage
        from bot.channels.middleware import RateLimitMiddleware

        async def _test():
            mw = RateLimitMiddleware(max_per_hour=2)
            for i in range(2):
                msg = InboundMessage(
                    channel="test", sender_id="user1", chat_id="c",
                    content=f"msg{i}",
    ...
    def test_admin_bypasses_limit(self):
        from bot.channels.bus import InboundMessage
        from bot.channels.middleware import RateLimitMiddleware

        async def _test():
            mw = RateLimitMiddleware(max_per_hour=1, admin_ids={"admin1"})
            for i in range(10):
                msg = InboundMessage(
                    channel="test", sender_id="admin1", chat_id="c",
                    content=f"msg{i}",
                )
                result = await mw.process(msg)
                assert result is not None
    ...
    def test_zero_limit_allows_all(self):
        ...
        async def _test():
            mw = RateLimitMiddleware(max_per_hour=0)
            for i in range(100):
                msg = InboundMessage(
                    channel="test", sender_id="user1", chat_id="c",
                    content=f"msg{i}",
...
class TestAllowListMiddleware:
    """Test AllowListMiddleware."""

    def test_allowed_sender_passes(self):
        from bot.channels.bus import InboundMessage
        from bot.channels.middleware import AllowListMiddleware

        async def _test():
            mw = AllowListMiddleware(allowed_senders={"admin"})
            msg = InboundMessage(
                channel="test", sender_id="admin", chat_id="c", content="hello",
            )
            result = await mw.process(msg)
            assert result is not None
    ...
    def test_disallowed_sender_dropped(self):
        from bot.channels.bus import InboundMessage
        from bot.channels.middleware import AllowListMiddleware
        ...
        async def _test():
            mw = AllowListMiddleware(allowed_senders={"admin"})
            msg = InboundMessage(
                channel="test", sender_id="stranger", chat_id="c", content="hello",
            )
            result = await mw.process(msg)
            assert result is None
    ...
    def test_no_allow_list_passes_all(self):
        from bot.channels.bus import InboundMessage
        from bot.channels.middleware import AllowListMiddleware
        ...
        async def _test():
            ...
            msg = InboundMessage(
                channel="test", sender_id="anyone", chat_id="c", content="hello",
            )
            result = await mw.process(msg)
            assert result is not None
...
class TestMentionGatingMiddleware:
    """Test MentionGatingMiddleware."""

    def test_dm_always_passes(self):
        from bot.channels.bus import InboundMessage
        from bot.channels.middleware import MentionGatingMiddleware

        async def _test():
            mw = MentionGatingMiddleware(require_mention="group")
            msg = InboundMessage(
                channel="test", sender_id="u", chat_id="c", content="hello",
    ...
    def test_group_without_mention_dropped(self):
        from bot.channels.bus import InboundMessage
        from bot.channels.middleware import MentionGatingMiddleware
    ...
    def test_group_with_mention_passes(self):
        from bot.channels.bus import InboundMessage
        from bot.channels.middleware import MentionGatingMiddleware
    ...
    def test_off_policy_passes_all(self):
        from bot.channels.bus import InboundMessage
        from bot.channels.middleware import MentionGatingMiddleware

        async def _test():
            mw = MentionGatingMiddleware(require_mention="off")
            msg = InboundMessage(
                channel="test", sender_id="u", chat_id="c", content="hello",
                is_group=True, was_mentioned=False,
            )
...
class TestTextLimitMiddleware:
    """Test TextLimitMiddleware."""

    def test_short_text_passes(self):
        from bot.channels.bus import OutboundMessage
        from bot.channels.middleware import TextLimitMiddleware

        async def _test():
            mw = TextLimitMiddleware(max_length=100)
            msg = OutboundMessage(channel="test", chat_id="c", content="short")
            result = await mw.process(msg)
            assert result is not None
    ...
    def test_long_text_truncated(self):
        ...
        async def _test():
            mw = TextLimitMiddleware(max_length=50)
            msg = OutboundMessage(channel="test", chat_id="c", content="x" * 200)
            result = await mw.process(msg)
            assert result is not None
            assert len(result.content) <= 50
            assert "[Message truncated" in result.content

        asyncio.run(_test())


class TestMiddlewarePipeline:
    """Test the MiddlewarePipeline runner."""

    def test_chain_inbound_middleware(self):
        from bot.channels.bus import InboundMessage
        ...
        async def _test():
            pipeline = MiddlewarePipeline(
                inbound=[DedupMiddleware(), RateLimitMiddleware(max_per_hour=5)],
            )
            msg = InboundMessage(
                channel="test", sender_id="u1", chat_id="c",
                content="hello", message_id="m1",
            )
            result = await pipeline.process_inbound(msg)
            assert result is not None

            # Same message again -> dedup drops it
            result2 = await pipeline.process_inbound(msg)
            assert result2 is None

        asyncio.run(_test())

    def test_chain_outbound_middleware(self):
        from bot.channels.bus import OutboundMessage
        from bot.channels.middleware import (
            MiddlewarePipeline,
            TextLimitMiddleware,
        ...
        async def _test():
            pipeline = MiddlewarePipeline(
                outbound=[TextLimitMiddleware(max_length=50)],
            )
            msg = OutboundMessage(channel="test", chat_id="c", content="x" * 200)
            result = await pipeline.process_outbound(msg)
            assert result is not None
            assert len(result.content) <= 50
