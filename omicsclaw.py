...
def _finalize_output_directory(
    ...
    *,
...
) -> tuple[Path, str | None, str, dict[str, Any] | None]:
    ...
    return final_dir, actual_method, str(readme_path), payload
...
def _write_pipeline_readme(
    ...
    *,
    ...
    for step_name, info in results.items():
        ...
        lines.append(f"| `{step_name}` | `{status}` | `{method}` | `{output_name}` |")
...
def run_skill(
    ...
    *,
    ...
    if proc.returncode == 0:
        final_out_dir, actual_method, readme_path, _ = _finalize_output_directory(
            out_dir,
