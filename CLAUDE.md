...
**Note**: For backward compatibility, spatial transcriptomics users can still refer to you as "SpatialClaw" and all 16 spatial skills remain fully functional.
...
|---|---|---|
| Load spatial data, QC, preprocess, normalize, Visium, Xenium, MERFISH, Slide-seq, cluster | `skills/spatial/preprocess/` | Run `python omicsclaw.py run preprocess` |
| Spatial domains, tissue regions, niches, SpaGCN, STAGATE, GraphST | `skills/spatial/domains/` | Run `python omicsclaw.py run domains` |
| Cell type annotation, assign cell types, Tangram, scANVI, CellAssign, scType | `skills/spatial/annotate/` | Run `python omicsclaw.py run annotate` |
| Deconvolution, cell type proportions, CARD, Cell2Location, RCTD, FlashDeconv | `skills/spatial/deconv/` | Run `python omicsclaw.py run deconv` |
| Spatial statistics, autocorrelation, Moran's I, Geary, Ripley, neighborhood enrichment | `skills/spatial/statistics/` | Run `python omicsclaw.py run statistics` |
| Spatially variable genes, SpatialDE, SPARK-X, spatial gene patterns | `skills/spatial/genes/` | Run `python omicsclaw.py run genes` |
| Differential expression, marker genes, group comparison, Wilcoxon | `skills/spatial/de/` | Run `python omicsclaw.py run de` |
| Condition comparison, pseudobulk, DESeq2, experimental conditions | `skills/spatial/condition/` | Run `python omicsclaw.py run condition` |
| Cell communication, ligand-receptor, LIANA, CellPhoneDB, FastCCC | `skills/spatial/communication/` | Run `python omicsclaw.py run communication` |
| RNA velocity, cellular dynamics, scVelo, VeloVI | `skills/spatial/velocity/` | Run `python omicsclaw.py run velocity` |
| Trajectory inference, pseudotime, CellRank, Palantir, DPT | `skills/spatial/trajectory/` | Run `python omicsclaw.py run trajectory` |
| Pathway enrichment, GSEA, ORA, GO, KEGG, Reactome | `skills/spatial/enrichment/` | Run `python omicsclaw.py run enrichment` |
| Copy number variation, CNV, inferCNV | `skills/spatial/cnv/` | Run `python omicsclaw.py run cnv` |
| Multi-sample integration, batch correction, Harmony, BBKNN, Scanorama | `skills/spatial/integrate/` | Run `python omicsclaw.py run integrate` |
| Spatial registration, slice alignment, PASTE, STalign | `skills/spatial/register/` | Run `python omicsclaw.py run register` |

### Single-Cell Omics (5 skills)

| User Intent | Skill | Action |
|---|---|---|
| Single-cell QC, preprocess, normalize, cluster | `skills/singlecell/preprocessing/` | Run `python omicsclaw.py run sc-preprocess` |
| Doublet detection, remove doublets | `skills/singlecell/doublet-detection/` | Run `python omicsclaw.py run sc-doublet` |
| Single-cell trajectory, pseudotime | `skills/singlecell/trajectory/` | Run `python omicsclaw.py run sc-trajectory` |
| Single-cell annotation, cell types | `skills/singlecell/annotation/` | Run `python omicsclaw.py run sc-annotate` |
| Single-cell integration, batch correction | `skills/singlecell/integration/` | Run `python omicsclaw.py run sc-integrate` |

### Genomics (8 skills)

| User Intent | Skill | Action |
|---|---|---|
| VCF operations, variant statistics | `skills/genomics/vcf-ops/` | Run `python omicsclaw.py run vcf-ops` |
| Variant calling, call variants | `skills/genomics/variant-calling/` | Run `python omicsclaw.py run variant-call` |
| Genomics QC, quality control | `skills/genomics/qc/` | Run `python omicsclaw.py run genomics-qc` |
| Read alignment, align to reference | `skills/genomics/alignment/` | Run `python omicsclaw.py run align` |
| Variant annotation, annotate variants | `skills/genomics/annotation/` | Run `python omicsclaw.py run variant-annotate` |
| Structural variants, SV detection | `skills/genomics/structural-variants/` | Run `python omicsclaw.py run sv-detect` |
| Genome assembly, assemble reads | `skills/genomics/assembly/` | Run `python omicsclaw.py run assemble` |
| Haplotype phasing, phase variants | `skills/genomics/phasing/` | Run `python omicsclaw.py run phase` |

### Proteomics (6 skills)

| User Intent | Skill | Action |
|---|---|---|
| MS QC, mass spectrometry quality control | `skills/proteomics/ms-qc/` | Run `python omicsclaw.py run ms-qc` |
| Import proteomics data, convert formats | `skills/proteomics/data-import/` | Run `python omicsclaw.py run data-import` |
| Peptide identification, identify peptides | `skills/proteomics/peptide-id/` | Run `python omicsclaw.py run peptide-id` |
| Protein quantification, quantify proteins | `skills/proteomics/quantification/` | Run `python omicsclaw.py run quantification` |
| Differential abundance, compare proteins | `skills/proteomics/differential-abundance/` | Run `python omicsclaw.py run differential-abundance` |
| PTM analysis, post-translational modifications | `skills/proteomics/ptm/` | Run `python omicsclaw.py run ptm` |

### Metabolomics (5 skills)

| User Intent | Skill | Action |
|---|---|---|
| Peak detection, detect metabolite peaks | `skills/metabolomics/peak-detection/` | Run `python omicsclaw.py run peak-detect` |
| XCMS preprocessing, peak alignment | `skills/metabolomics/xcms-preprocess/` | Run `python omicsclaw.py run xcms-preprocess` |
